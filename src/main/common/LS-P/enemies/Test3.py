...
def check_collision(character_rect, item_rects):
    for item_rect in item_rects:
        if character_rect.colliderect(item_rect):
            ...
            return item_images.index(item_rect)
    return -1
...
while game_running:
    ...
    item_index = check_collision(character_rect, item_rects)
    if item_index >= 0:
    ...
    for i in range(4):
        item_display = font.render("Item {}: {}".format(i+1, inventory[i]), True, (0, 0, 0))
        game_window.blit(item_display, (10, 30 + 30*i))
