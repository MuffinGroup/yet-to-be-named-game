...
class registerElement:
    def __init__(self, elementLocation, scale):
        ...
        self.texture = pygame.image.load(
            "src/main/assets/textures/" + elementLocation + ".png"
        )
    ...
    def drawElement(self, surface, x, y, rectArray):
        self.rect = pygame.Rect(
            (x * 96 + self.xRectModifier, y * 96 + self.yRectModifier),
            (
                self.scaledTexture.get_width() + self.widthModifier,
                self.scaledTexture.get_height() + self.heightModifier,
            ),
        )
        surface.blit(
            self.scaledTexture, (x * 96 + self.xModifier, y * 96 + self.yModifier)
        )
    ...
    def drawRotatedElement(self, surface, x, y, flipped):
        if flipped == True:
            ...
            surface.blit(
                self.flippedTexture, (x * 96 + self.xModifier, y * 96 + self.yModifier)
            )
        else:
            surface.blit(
                self.scaledTexture, (x * 96 + self.xModifier, y * 96 + self.yModifier)
    ...
    def drawStairElement(self, surface, x, y, flippedX, flippedY, rectArray):
        if flippedX == False and flippedY == False:
            self.flippedTexture = pygame.transform.flip(
                self.scaledTexture, False, False
            )
            self.rect1 = pygame.Rect(
                (
                    x * 96 + self.xRectModifier,
                    y * 96 + self.scaledTexture.get_height() // 2 + self.yRectModifier,
                ),
                (
                    self.scaledTexture.get_width() + self.widthModifier,
                    self.scaledTexture.get_height() // 2 + self.heightModifier,
                ),
            )
            self.rect2 = pygame.Rect(
                (
                    x * 96 + self.scaledTexture.get_width() // 2 + self.xRectModifier,
                    y * 96 + self.yRectModifier,
                ),
                (
                    self.scaledTexture.get_width() // 2 + self.widthModifier,
                    self.scaledTexture.get_height() // 2 + self.heightModifier,
                ),
            )
            surface.blit(
    ...
    def drawPedestalElement(self, surface, x, y, rectArray):
        self.rect1 = pygame.Rect(
            (x * 96 + self.scale * 3 + self.xRectModifier, y * 96 + self.yRectModifier),
            (
                self.scaledTexture.get_width() - self.scale * 6 + self.widthModifier,
                self.scaledTexture.get_height() + self.heightModifier,
            ),
        )
        self.rect2 = pygame.Rect(
...
class registerInvisibleElement:
    ...
    def drawElement(self, x, y, rectArray):
        self.rect = pygame.Rect(
            (x * 96 + self.xRectModifier, y * 96 + self.yRectModifier),
            (96 + self.widthModifier, 96 + self.heightModifier),
        )
        rectArray.append(self.rect)
